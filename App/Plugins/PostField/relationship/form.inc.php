...
$form_text .= '<div class="col-tab">' .
    '<ul class="tabBut cu-li">';
        ...
        $form_text .= '<li id="tab_setting_<?php echo $key +1; ?>" class="' . ($key == 0 ? 'on' : '') . '" onclick="SwapTab(\'setting\',\'on\',\'\', '. $model_num. ' ,' . ($key + 1) . ');">'. $model['name'] .'</li>';
        }
    $form_text .= '</ul>';
    foreach ($models as $key => $model) {
    $form_text .= '<div id="div_setting_' . ($key + 1) . '" class="contentList pad-10 field ' . ($key == 0 ? '' : 'hidden') . '">' .
        '<div class="cfs_relationship">' .
            ...
            '</div>' .
        '</div>' .
        '<input type="hidden" name="info[relationship]['. $model['tablename'] .']" class="relationship" value="' . (isset($value[$model['tablename']]) ? $value[$model['tablename']] : '') . '">' .
        '</div>';
    }
